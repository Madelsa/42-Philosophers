...
/*   Updated: 2024/03/15 15:02:04 by mabdelsa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	confirm_threads_running(t_data *philo_data)
{
	while (1)
	{
		if (threads_running(&philo_data->data_mutex,
				&philo_data->no_of_threads_running,
				philo_data->no_of_philos) == 1)
			break ;
	}
}
...
void	*monitor_sim(void *data)
{
	...
	while (simulation_ended(philo_data) == 0)
	{
			...
			&& simulation_ended(philo_data) == 0)
		{
			...
			else if (get_int(&philo_data->philos->philos_mutex,
					&philo_data->philos->is_full) == 1)
				set_int(&philo_data->data_mutex, &philo_data->sim_ended, 1);
			i++;
		}
	}
