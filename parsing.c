...
/*   Updated: 2024/03/10 11:13:42 by mahmoud          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int validate_input(char **av)
{
    ...
    if (av[1][0] == '-' || ft_atol(av[1]) == 0)
        return (ft_putstr_fd
                ("\033[1;31mNumber of Philosophers must be greater than 0.\033[0m\n", 2), 1);
    while (av[i] != NULL)
    {
        if ((ft_strcmp(av[i], "0") != 0 && ft_atol(av[i]) == 0)
		    || (ft_strcmp(av[i], "-1") != 0 && ft_atol(av[i]) == -1))
            return (ft_putstr_fd
                ("\033[1;31mInput overflowed (> INT_MAX or < 0).\033[0m\n", 2), 1);
        if (ft_atol(av[i]) > INT_MAX || ft_atol(av[i]) < 0)
            return (ft_putstr_fd
                ("\033[1;31mValue larger than INT_MAX or smaller than 0.\033[0m\n", 2), 1);
