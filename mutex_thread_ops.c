...
/*   Updated: 2024/03/15 15:02:39 by mabdelsa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	mutex_ops(pthread_mutex_t *mutex, char *operation)
{
	if (ft_strcmp(operation, "LOCK") == 0)
		...
		return (ft_putstr_fd("\033[1;31mOperation not found.\033[0m\n", 2));
}

void	thread_ops(pthread_t *thread, void *(*func)(void *), void *data,
		char *operation)
{
